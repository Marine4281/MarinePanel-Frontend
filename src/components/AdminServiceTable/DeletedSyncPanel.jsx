// src/components/AdminServiceTable/DeletedSyncPanel.jsx
...
export default function DeletedSyncPanel({ onDeleted }) {
  ...
  const fetchDeleted = useCallback(async () => {
    ...
    setSelected([]);
    try {
      // 🔁 update this path to match wherever you registered the route
      const res = await API.get("/admin/services/deleted-sync");
      setDeleted(res.data || []);
    } catch {
  ...
  const removeSelected = async () => {
    ...
    try {
      ...
      setSelected([]);
      toast.success(`${removed} service(s) removed`);
      onDeleted?.();
    } catch {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Find services removed from your providers that still exist in your catalog.
          </p>
        </div>
        <button
