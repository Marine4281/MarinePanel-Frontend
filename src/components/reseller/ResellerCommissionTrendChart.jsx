// src/components/reseller/ResellerCommissionTrendChart.jsx
...
const ResellerCommissionTrendChart = () => {
  ...
  const fetchTrend = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await API.get("/reseller/commission-trend", {
        params: { range },
      });
      setLabels(Array.isArray(data?.labels) ? data.labels : []);
      setValues(Array.isArray(data?.data) ? data.data : []);
    } catch (err) {
      console.error("Failed to fetch commission trend", err);
      setLabels([]);
      setValues([]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          <h3 className="text-lg font-semibold text-gray-800">Commission Earned</h3>
      ...
      <div className="h-72">
        {loading ? (
          ...
          >
            ...
            <path d={areaPath} fill="url(#resellerCommissionFill)" stroke="none" />
...
export default ResellerCommissionTrendChart;
