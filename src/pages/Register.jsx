...
const Register = () => {
  ...
  const branding =
    domainType === "childPanel" && childPanel
      ...
      ? {
          ...
          themeColor: reseller.themeColor || "#f97316",
        }
      : {
  ...
  if (isLoading) {
    return (
      <div
        ...
        style={{
          background:
            domainType === "main" ? "#f8fafc" : "#0f172a",
        }}
      >
        <div
          className="animate-pulse w-full max-w-md p-8 rounded-3xl"
          style={{ background: domainType === "main" ? "#e5e7eb" : "rgba(255,255,255,0.05)" }}
        >
  ...
  if (domainType === "childPanel") {
    return (
      ...
      >
        ...
        <div className="flex-1 flex items-start justify-center px-4 pb-10">
          <div className="w-full max-w-md bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl shadow-2xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-white/70 text-xs font-semibold mb-1">Phone</label>
                <PhoneInput country={phoneCountry} value={phone} enableSearch searchPlaceholder="Search country..."
                  disableCountryGuess={true} countryCodeEditable={false} preferredCountries={["ke", "us", "gb"]}
                  onChange={(value, data) => { setPhone(value); const iso2 = (data?.countryCode || "us").toLowerCase().trim(); setPhoneCountry(iso2); setCountry({ name: data?.name || "", code: iso2.toUpperCase() }); }}
                  containerClass="w-full" inputClass="!w-full !pl-14 !pr-3 !py-3 !rounded-xl !border !border-white/30 !bg-white/10 !text-white !text-sm"
                  buttonClass="!border !border-white/30 !rounded-l-xl !px-2 !bg-white/10" dropdownClass="!rounded-xl !shadow-lg" />
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/60 text-xs hover:text-white">
                    {showPassword ? "Hide" : "Show"}
                  </button>
                </div>
              </div>
              <div>
                <label className="block text-white/70 text-xs font-semibold mb-1">Confirm Password</label>
                <div className="relative">
                  <input type={showConfirm ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required
                    className="w-full bg-white/10 border border-white/30 text-white placeholder-white/40 rounded-xl px-4 py-3 pr-16 text-sm focus:outline-none focus:ring-2 focus:ring-white/50" />
                  <button type="button" onClick={() => setShowConfirm((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/60 text-xs hover:text-white">
              ...
              <p className="text-white/40 text-xs text-center leading-relaxed">
                ...
                <Link to="/terms-public" target="_blank" rel="noopener noreferrer" className="text-white/70 underline underline-offset-2 hover:text-white">
                  Terms & Conditions
                </Link>
              </p>
        ...
        <p className="text-center text-white/30 text-xs py-4">© {new Date().getFullYear()} {branding.brandName}. All rights reserved.</p>
      </div>
    );
  }

  // ======================= RESELLER REGISTER =======================
  // Same dark glass UI as ResellerLogin — consistent look within the reseller domain.
      ...
      >
        ...
        <div className="relative w-full max-w-[420px]">
          ...
          <div className="rounded-3xl p-8"
            style={{ background: "rgba(255,255,255,0.04)", backdropFilter: "blur(32px)", WebkitBackdropFilter: "blur(32px)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: `0 0 80px ${tc}14, 0 32px 64px rgba(0,0,0,0.5)` }}>
            <div className="mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(255,255,255,0.5)" }}>Phone Number</label>
                <PhoneInput country={phoneCountry} value={phone} enableSearch searchPlaceholder="Search country..."
                  disableCountryGuess={true} countryCodeEditable={false} preferredCountries={["ke", "us", "gb"]}
                  onChange={(value, data) => { setPhone(value); const iso2 = (data?.countryCode || "us").toLowerCase().trim(); setPhoneCountry(iso2); setCountry({ name: data?.name || "", code: iso2.toUpperCase() }); }}
                  containerClass="w-full" inputClass="!w-full !pl-14 !pr-3 !py-3 !rounded-xl !text-sm !outline-none"
                  buttonClass="!rounded-l-xl !px-2" dropdownClass="!rounded-xl !shadow-lg" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPassword((s) => !s)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>
                    {showPassword ? "Hide" : "Show"}
                  </button>
                </div>
              </div>
              ...
              <p className="text-xs text-center leading-relaxed" style={{ color: "rgba(255,255,255,0.3)" }}>
                By registering you accept our{" "}
                <Link to="/terms-public" target="_blank" rel="noopener noreferrer"
                  className="underline underline-offset-2 hover:opacity-100" style={{ color: tc, opacity: 0.8 }}>
                  Terms & Conditions
                </Link>
              </p>
              ...
              <button type="submit" disabled={loading}
                ...
                style={{ background: `linear-gradient(135deg, ${tc} 0%, ${tc}cc 100%)`, color: "#fff", boxShadow: `0 4px 24px ${tc}40` }}>
                {loading ? "Creating account..." : "Create Account"}
              </button>
            </form>
            ...
            <p className="text-center text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>
              Already have an account?{" "}
              <Link to="/login" className="font-semibold hover:underline" style={{ color: tc }}>Sign in</Link>
            </p>
          </div>
  ...
  return (
    ...
    >
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -top-60 -left-40 w-[500px] h-[500px] rounded-full opacity-[0.06] blur-3xl" style={{ background: tc }} />
        <div className="absolute -bottom-60 -right-40 w-[500px] h-[500px] rounded-full opacity-[0.04] blur-3xl" style={{ background: "#6366f1" }} />
      </div>
      ...
      <div className="relative w-full max-w-[420px]">
        <div className="flex items-center justify-center gap-3 mb-7">
          {branding.logo ? (
            ...
            <div className="h-9 w-9 rounded-xl flex items-center justify-center text-base font-black text-white"
              style={{ background: `linear-gradient(135deg, ${tc}, ${tc}99)` }}>
        ...
        <div className="rounded-3xl p-8"
          ...
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white mb-1">Create account</h2>
            <p className="text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>Join {branding.brandName} and start growing today</p>
          </div>
