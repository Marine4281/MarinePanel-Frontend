...
const FEATURES = [
  { icon: "⚡", title: "Sub-second start",    desc: "Orders begin processing the moment you submit. No queue, no delay." },
  { icon: "💸", title: "Wholesale pricing",   desc: "Direct provider rates. The lowest prices you'll find, guaranteed." },
  { icon: "🛡️", title: "Zero-risk delivery",  desc: "No passwords ever. Your accounts stay safe, always." },
  { icon: "📊", title: "Live order tracking", desc: "Watch your order progress update in real time from the dashboard." },
  { icon: "🔌", title: "Full API access",     desc: "Automate everything with our developer-friendly REST API." },
  { icon: "🌍", title: "24/7 support",        desc: "Real humans, fast replies, any time of day or night." },
];

const REVIEWS = [
  { name: "Marcus D.",  role: "Agency Owner",   text: "I run 12 client accounts through this panel. Saves me hours every single day." },
  { name: "Lena K.",    role: "Influencer",     text: "Prices are insane. I've tried 6 other panels — nothing comes close." },
  { name: "Tyler R.",   role: "Freelancer",     text: "Set up in 5 minutes. First order delivered before I even refreshed the page." },
];
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans flex flex-col">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-12">
            <p className="text-gray-400 text-lg max-w-md leading-relaxed">
              {brandName} is the professional's choice for SMM services. Fast, reliable, and cheaper than anywhere else on the market.
            </p>
            <div className="flex gap-4 shrink-0">
              <Link to="/register"
                className="px-8 py-4 rounded-xl font-black text-white text-base hover:opacity-90 transition-all shadow-xl"
                style={{ backgroundColor: tc }}>
                Get Started →
              </Link>
              <Link to="/login"
      ...
      <section className="px-8 md:px-16 py-20">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl font-black text-gray-900 mb-14">Real users. Real results.</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {REVIEWS.map((r) => (
              <div key={r.name} className="rounded-2xl p-7 border-2 border-gray-100 hover:border-gray-200 transition-all">
                <div className="flex gap-1 mb-4">
                  {"★★★★★".split("").map((s, i) => (
                    <span key={i} style={{ color: tc }}>{s}</span>
                  ))}
                </div>
      ...
      <section className="px-8 md:px-16 pb-20">
        <div className="max-w-6xl mx-auto rounded-3xl p-12 md:p-16 relative overflow-hidden border-2"
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            ...
            <Link to="/register"
              className="shrink-0 px-12 py-5 rounded-2xl font-black text-xl hover:scale-105 transition-all shadow-2xl"
