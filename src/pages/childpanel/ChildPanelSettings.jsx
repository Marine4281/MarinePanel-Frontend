...
export default function ChildPanelSettings() {
  ...
  // SEO state survives tab switching
  ...
  const handleSaved = async (patch) => {
    ...
    if (
      updateUser &&
      (patch.brandName || patch.themeColor || patch.logo)
    ) {
      updateUser({
        childPanelBrandName:
          patch.brandName ??
          settings?.childPanelBrandName,

        childPanelThemeColor:
          patch.themeColor ??
          settings?.childPanelThemeColor,

        childPanelLogo:
          patch.logo ??
          settings?.childPanelLogo,
      });
    }

    // Refresh Child Panel branding context
    if (refetchBranding) {
      try {
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Configure your panel branding, domain,
            fees, gateway, templates, SEO and billing.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          {activeTab === "seo" && (
            <SeoTab
              ...
              setCpSeo={setCpSeo}
              cpOgImage={cpOgImage}
              setCpOgImage={setCpOgImage}
              cpFavicon={cpFavicon}
              setCpFavicon={setCpFavicon}
            />
          )}
        </div>
      </div>
