...
export default function NeonRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (pw !== confirm) return toast.error("Keys don't match");
      ...
      <div className="relative w-full max-w-sm">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" required style={inputStyle}
                onFocus={(e) => (e.target.style.borderColor = neon)} onBlur={(e) => (e.target.style.borderColor = `${neon}25`)} />
            </div>
            ...
            <div className="relative">
              ...
              <input type={showPw ? "text" : "password"} value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Access key" required
            ...
            <div className="relative">
              <FiLock size={13} className="absolute left-4 top-3.5" style={{ color: `${neon}55` }} />
              <input type={showCon ? "text" : "password"} value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="Confirm key" required
                style={{ ...inputStyle, paddingRight: 40 }}
                onFocus={(e) => (e.target.style.borderColor = neon)} onBlur={(e) => (e.target.style.borderColor = `${neon}25`)} />
            ...
            <button type="submit" disabled={loading} className="w-full py-3 rounded-xl text-sm font-black mt-1 disabled:opacity-60"
              style={{ background: `linear-gradient(135deg, ${neon}cc, ${neon}88)`, color: "#0a0a14", boxShadow: `0 0 20px ${neon}40` }}>
          ...
          <p className="text-center text-xs" style={{ color: "#3a3a5a" }}>
            Have access?{" "}
            <Link to="/login" className="font-bold hover:underline" style={{ color: neon }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
