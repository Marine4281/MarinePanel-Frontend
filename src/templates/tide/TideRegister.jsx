...
export default function TideRegister() {
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl text-sm border border-gray-200 bg-gray-50 outline-none text-gray-800 transition-colors";
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            {/* Banner */}
            <div className="px-8 py-6 text-center" style={{ background: `linear-gradient(135deg, ${brand.color}, ${brand.color}cc)` }}>
              {brand.logo ? (
                <img src={brand.logo} alt="" className="w-12 h-12 rounded-xl object-contain mx-auto mb-2 shadow" />
              ) : (
                <div className="w-12 h-12 rounded-xl bg-white/20 mx-auto mb-2 flex items-center justify-center text-2xl font-black text-white shadow">
                  {brand.name[0]}
                </div>
              )}
            ...
            <form onSubmit={handleSubmit} className="px-8 py-6 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <PhoneInput country="us" value={phone} onChange={setPhone}
                  inputStyle={{ width: "100%", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 12, fontSize: 14, height: 46, paddingLeft: 52, color: "#1f2937" }}
                  buttonStyle={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRight: "none", borderRadius: "12px 0 0 12px" }} />
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input type={showPw ? "text" : "password"} value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Min 6 characters" required
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <FiLock size={14} className="absolute left-4 top-3.5 text-gray-300" />
                  <input type={showCon ? "text" : "password"} value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="Repeat password" required
                    className={`${inputClass} pl-10 pr-10`}
                    onFocus={(e) => (e.target.style.borderColor = brand.color)} onBlur={(e) => (e.target.style.borderColor = "#e5e7eb")} />
                  <button type="button" onClick={() => setShowCon((s) => !s)} className="absolute right-4 top-3.5 text-gray-300">
                    {showCon ? <FiEyeOff size={14} /> : <FiEye size={14} />}
                  </button>
                </div>
              </div>
              ...
              <p className="text-center text-sm text-gray-500">
                Already registered?{" "}
                <Link to="/login" className="font-bold hover:underline" style={{ color: brand.color }}>Sign in</Link>
              </p>
            </form>
          </div>
        </div>
